...
pub async fn create_dataset(configuration: &configuration::Configuration, create_dataset_request: models::CreateDatasetRequest, x_database_id: Option<&str>) -> Result<models::CreateDatasetResponse, Error<CreateDatasetError>> {
...
pub async fn list_dataset_versions(configuration: &configuration::Configuration, id: &str, limit: Option<i32>, offset: Option<i32>) -> Result<models::ListDatasetVersionsResponse, Error<ListDatasetVersionsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_id = id;
    let p_query_limit = limit;
    let p_query_offset = offset;

    let uri_str = format!("{}/v1/datasets/{id}/versions", configuration.base_path, id=crate::apis::urlencode(p_path_id));
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
