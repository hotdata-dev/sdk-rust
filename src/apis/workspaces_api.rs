...
pub async fn create_workspace(configuration: &configuration::Configuration, create_workspace_request: models::CreateWorkspaceRequest) -> Result<models::CreateWorkspace201Response, Error<CreateWorkspaceError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::CreateWorkspace201Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::CreateWorkspace201Response`")))),
        }
    } else {
        let content = resp.text().await?;
...
pub async fn list_workspaces(configuration: &configuration::Configuration, organization_public_id: Option<&str>) -> Result<models::ListWorkspaces200Response, Error<ListWorkspacesError>> {
    // add a prefix to parameters to efficiently prevent name collisions
