...
pub async fn delete_workspace(configuration: &configuration::Configuration, public_id: &str) -> Result<(), Error<DeleteWorkspaceError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_public_id = public_id;

    let uri_str = format!("{}/v1/workspaces/{public_id}", configuration.base_path, public_id=crate::apis::urlencode(p_path_public_id));
