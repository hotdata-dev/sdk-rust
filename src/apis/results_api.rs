...
/// struct for typed errors of method [`get_result_handler`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetResultHandlerError {
    Status404(models::ApiErrorResponse),
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`list_results_handler`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ListResultsHandlerError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_result_handler(configuration: &configuration::Configuration, id: &str) -> Result<models::GetResultResponse, Error<GetResultHandlerError>> {
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetResultHandlerError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}

pub async fn list_results_handler(configuration: &configuration::Configuration, limit: Option<i32>, offset: Option<i32>) -> Result<models::ListResultsResponse, Error<ListResultsHandlerError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ListResultsHandlerError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
